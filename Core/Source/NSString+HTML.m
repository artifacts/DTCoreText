...
static NSDictionary *entityLookupXML = nil;
static NSDictionary *entityLookupHTML = nil;
static NSDictionary *entityReverseLookupXML = nil;
static NSDictionary *entityReverseLookupHTML = nil;
...
{
    return [self stringByAddingEntitiesEscapeHTML:YES];
}

- (NSString *)stringByAddingXMLEntities
{
    return [self stringByAddingEntitiesEscapeHTML:NO];
}

- (NSString *)stringByAddingEntitiesEscapeHTML:(BOOL)escapeHTML
{
    [self prepareReverseLookup];
	...
	for (NSUInteger i = 0; i<[self length]; i++)
	{
		...
		NSString *entity = [entityReverseLookupXML objectForKey:subKey];
        if(!entity && escapeHTML) {
            entity = [entityReverseLookupHTML objectForKey:subKey];
        }
		...
		{
            if((escapeHTML && oneChar<=255) || (!escapeHTML && (oneChar > 31 && oneChar <= 125)))
			{
				// output as is
...
- (NSString *)stringByReplacingHTMLEntities
{
    ...
    dispatch_once(&predicate, ^{
        entityLookupXML = [[NSDictionary alloc] initWithObjectsAndKeys:
                           @"\x22", @"quot",
                           @"\x26", @"amp",
                           @"\x27", @"apos",
                           @"\x3c", @"lt",
        ...
        if ([scanner scanString:@"&" intoString:NULL])
        {
            ...
            if ([scanner scanUpToString:@";" intoString:&afterAmpersand])
            {
                if ([scanner scanString:@";" intoString:NULL])
                {
                    ...
                    {
                        NSString *converted = [entityLookupXML objectForKey:afterAmpersand];
    ...
    while (![scanner isAtEnd])
    {
        ...
        if ([scanner scanCharactersFromSet:spaceSet intoString:&spaces])
        {
            ...
            if (numSpaces > 0)
            {
                ...
                for (NSUInteger i=0; i<numSpaces - 1;i++)
                {
                    if (i%2)
                    {
                        [output appendString:@" "];
...
- (void) prepareReverseLookup {
    ...
    dispatch_once(&predicate, ^{
        entityReverseLookupXML = [[NSDictionary alloc] initWithObjectsAndKeys:
        ...
        entityReverseLookupHTML = [[NSDictionary alloc] initWithObjectsAndKeys:
                                   @"&#xA0;", [NSNumber numberWithInteger:0x00a0],
